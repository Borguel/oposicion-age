...
oficiales de GACE en Firestore (colección `examenes_oficiales_GACE`).

Cada pregunta se asignó manualmente a uno de los 58 temas del temario oficial
de GACE (ver cargar_temario_boe.py), en base al artículo/ley que cita, y esa
asignación vive en datos_examenes/gacel_<anio>_1er_ejercicio_temas.json
(mapa "numero" -> "bloque_XX-tema_XX").

Requiere las mismas variables de entorno que el resto del proyecto:
FIREBASE_CREDENTIALS_JSON (o FIREBASE_KEY_PATH).

Uso:
    python etiquetar_temas_examenes_gace.py <anio>
    python etiquetar_temas_examenes_gace.py 2022
"""
import os
import sys
import json
...
def etiquetar(anio):
    ruta_temas = os.path.join(BASE_DIR, "datos_examenes", f"gacel_{anio}_1er_ejercicio_temas.json")
    ...
    for numero_str, tema_id in mapa_temas.items():
        ...
        doc_id = f"gacel_{anio}_1ej_{numero:03d}"
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Uso: python etiquetar_temas_examenes_gace.py <anio>")
        sys.exit(1)
