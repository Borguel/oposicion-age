...
// de páginas del sitio. anime.js en sí se importa de forma perezosa, solo
// en la rama que de verdad anima (ver más abajo): quien prefiere menos
// movimiento llega igual al desenlace sin descargarlo, fijando los 4
// estilos finales a mano.
...
if (escena) {
  ...
  if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
    hojas.forEach((hoja) => { hoja.style.opacity = "0"; });
    pregunta.style.opacity = "1";
    check.style.strokeDashoffset = "0";
    opcionCorrecta.classList.add("correcta");
  } else {
    import("/assets/vendor/anime.esm.js").then(({ default: anime }) => {
      anime.set(hojas, {
        opacity: 1,
        rotate: (el, i) => [-9, 7, -3][i],
