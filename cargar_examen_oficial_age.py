"""Carga el Ejercicio Único de una convocatoria AGE (Cuerpo General
Administrativo de la Administración del Estado, Ingreso Libre) en Firestore,
bajo la colección de exámenes oficiales de AGE.

Las preguntas (70 + 5 de reserva) deben estar ya extraídas y revisadas en
datos_examenes/age_<anio>_ejercicio_unico.json. La explicación de cada
respuesta correcta se toma de datos_examenes/age_<anio>_ejercicio_unico_explicaciones.json
...
Uso:
    python cargar_examen_oficial_age.py <anio>
    python cargar_examen_oficial_age.py 2025
"""
import os
import sys
import json
...
def cargar_examen(anio):
    ruta_datos = os.path.join(BASE_DIR, "datos_examenes", f"age_{anio}_ejercicio_unico.json")
    ruta_explicaciones = os.path.join(
        BASE_DIR, "datos_examenes", f"age_{anio}_ejercicio_unico_explicaciones.json"
    )
    nombre_examen = f"AGE {anio} - Ejercicio único (ingreso libre)"
    ...
    for p in preguntas:
        ...
        doc_id = f"age_{anio}_eu_{p['numero']:03d}"
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Uso: python cargar_examen_oficial_age.py <anio>")
        sys.exit(1)
